...
git fetch template
git merge template/main --allow-unrelated-histories -m "Initial import from MGraph-AI__Service__Deploy template"
print_success "Template merged"

# Step 5: Rename the service directory
print_step "Renaming service directory from mgraph_ai_service_deploy to $SERVICE_NAME_UNDERSCORES..."
if [ -d "mgraph_ai_service_deploy" ]; then
    mv mgraph_ai_service_deploy "$SERVICE_NAME_UNDERSCORES"
    print_success "Service directory renamed"
else
    print_error "mgraph_ai_service_deploy directory not found!"
    exit 1
fi
...
# Replace "MGraph AI Service Deploy" with the new service title
replace_in_files "MGraph AI Service Deploy" "$SERVICE_NAME_TITLE" "service title"

# Replace "mgraph_ai_service_deploy" with the new package name
replace_in_files "mgraph_ai_service_deploy" "$SERVICE_NAME_UNDERSCORES" "Python package name"

# Replace "MGraph-AI__Service__Deploy" with the new repository name
replace_in_files "MGraph-AI__Service__Deploy" "$REPO_NAME" "repository name"
...
- Based on MGraph-AI__Service__Deploy v0.3.0
