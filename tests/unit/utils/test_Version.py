import mgraph_ai_service_deploy
...
class test_Version(TestCase):
    ...
    def test_path_code_root(self):
        assert self.version.path_code_root() == mgraph_ai_service_deploy.path

    def test_path_version_file(self):
        with self.version as _:
            assert parent_folder(_.path_version_file()) == mgraph_ai_service_deploy.path
            assert file_name    (_.path_version_file()) == 'version'

    def test_value(self):
        assert self.version.value() == version__mgraph_ai_service_deploy